...
export class AppComponent {
  ...
  async getRepos(page: number): Promise<void> {
    ...
    try {
      const res: { data: Repo[]; headers: any } =
        await this.apiService.getRepositories(
      ...
      if (error.status === 403) this.toast.showError(userMessages.FORBIDDEN);
